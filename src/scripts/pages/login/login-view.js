export default class LoginView {
    ...
    return `
      <section class="min-h-[calc(100vh-80px)] flex items-center justify-center bg-slate-50 relative overflow-hidden p-6">
        ...
        <div class="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
          ...
          <div class="absolute -bottom-24 -right-24 w-96 h-96 bg-blue-100 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse delay-700"></div>
        </div>
        ...
        <div class="glass-panel w-full max-w-lg p-10 md:p-16 rounded-[3rem] relative z-10 shadow-2xl shadow-primary-900/5">
          <div class="text-center mb-12">
            ...
            <h1 class="text-4xl font-black text-slate-900 mb-3 uppercase tracking-tighter">Selamat Datang</h1>
            <p class="text-slate-500 font-bold text-lg">Masuk untuk menjelajahi cerita dunia.</p>
          </div>

          <form id="storiapp-login-form" class="space-y-8" novalidate>
            <div class="space-y-3">
            ...
            <button type="submit" class="btn-primary w-full py-5 text-lg uppercase tracking-widest font-black" id="storiapp-login-btn">
          ...
          <div class="mt-12 text-center pt-10 border-t border-slate-100">
            <p class="text-slate-500 font-bold">
              Belum punya akun? 
              <a href="#/register" class="text-primary-600 font-black hover:underline underline-offset-4 ml-1">Daftar sekarang</a>
            </p>
          </div>
        </div>
      </section>
  ...
  setSubmitLoading(isLoading) {
    const btn = document.querySelector("#storiapp-login-btn");
  ...
  bindFormSubmit(handler) {
    const form = document.querySelector("#storiapp-login-form");
    if (!form) return;
